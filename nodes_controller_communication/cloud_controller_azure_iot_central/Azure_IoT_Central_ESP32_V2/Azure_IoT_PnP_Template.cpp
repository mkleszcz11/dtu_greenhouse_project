...
/* --- Defines --- */
#define AZURE_PNP_MODEL_ID "dtmi:centralApplicationNameTest:DOITESP32_592;1"
...
#define SAMPLE_MANUFACTURER_PROPERTY_VALUE "DOIT ESP32"
#define SAMPLE_MODEL_PROPERTY_VALUE "DOIT ESP32"
...
#define SAMPLE_PROCESSOR_MANUFACTURER_PROPERTY_VALUE "DOIT ESP32"
...
static az_span COMMAND_NAME_TEMP_MIN = AZ_SPAN_FROM_STR("SetTemperatureMinimum");
static az_span COMMAND_NAME_TEMP_MAX = AZ_SPAN_FROM_STR("SetTemperatureMaximum");
static az_span COMMAND_NAME_HUMIDITY_MIN = AZ_SPAN_FROM_STR("SetHumidityMinimum");
static az_span COMMAND_NAME_HUMIDITY_MAX = AZ_SPAN_FROM_STR("SetHumidityMaximum");
static az_span COMMAND_NAME_WATER_LEVEL_MIN = AZ_SPAN_FROM_STR("SetWaterLevelMinimum");
static az_span COMMAND_NAME_WATER_LEVEL_MAX = AZ_SPAN_FROM_STR("SetWaterLevelMaximum");
static az_span COMMAND_NAME_SOIL_MOISTURE_MIN = AZ_SPAN_FROM_STR("SetSoilMoistureMinimum");
static az_span COMMAND_NAME_SOIL_MOISTURE_MAX = AZ_SPAN_FROM_STR("SetSoilMoistureMaximum");
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command) {
  ...
  int payload_size = az_span_size(command.payload);
  //const uint8_t* payload_ptr = az_span_ptr(command.payload);
  uint8_t* payload_ptr = az_span_ptr(command.payload);
  ...
  if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_LED_1)) {
    ...
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TEMP_MIN)) {

    if (payload_size == 0) {
      LogError("Payload size was 0");
      response_code = COMMAND_RESPONSE_CODE_REJECTED;
      //fresh_data = false;
    }
    // Null-terminate the payload to convert it to a string
    char temp_str[32];
    int temp_index = 0;
    for (int i = 0; i < payload_size; i++) {
      // Copy only digits to the temp_str
      temp_str[temp_index++] = payload_ptr[i];
    }
    temp_str[temp_index] = '\0';  // Null-terminate the string

    // Convert the string to a uint8_t
    uint8_t min_temp = (uint8_t)atoi(temp_str);

    LogInfo("Minimum Temperature set to: %u", min_temp);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TEMP_MAX)) {
    ...
    // Convert the string to a uint8_t
    uint8_t max_temp = (uint8_t)atoi(temp_str);

    LogInfo("Maximum Temperature set to: %u", max_temp);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_HUMIDITY_MIN)) {
    ...
    // Convert the string to a uint8_t
    uint8_t min_humid = (uint8_t)atoi(temp_str);

    LogInfo("Minimum Humidity set to: %u", min_humid);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_HUMIDITY_MAX)) {
    ...
    // Convert the string to a uint8_t
    uint8_t max_humid = (uint8_t)atoi(temp_str);

    LogInfo("Maximum Humidity set to: %u", max_humid);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
    } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_WATER_LEVEL_MIN)) {
    ...
    // Convert the string to a uint8_t
    uint8_t min_water_level = (uint8_t)atoi(temp_str);

    LogInfo("Minimum Water Level set to: %u", min_water_level);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_WATER_LEVEL_MAX)) {
    ...
    // Convert the string to a uint8_t
    uint8_t max_water_level = (uint8_t)atoi(temp_str);

    LogInfo("Maximum Water Level set to: %u", max_water_level);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
} else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_SOIL_MOISTURE_MIN)) {
    ...
    // Convert the string to a uint8_t
    uint8_t min_soil_moisture = (uint8_t)atoi(temp_str);

    LogInfo("Minimum Soil Moisture set to: %u", min_soil_moisture);

    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  } else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_SOIL_MOISTURE_MAX)) {
    ...
    // Convert the string to a uint8_t
    uint8_t max_soil_moisture = (uint8_t)atoi(temp_str);

    LogInfo("Maximum Soil Moisture set to: %u", max_soil_moisture);
