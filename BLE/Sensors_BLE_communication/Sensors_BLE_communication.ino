...
float dynamicDelay=1000*60*5; // 5 minutes
...
static void notifyCallback(
  ...
  /*
    if floatSignalValue = -2 Sensor must go to Sleep Mode
    if floatSignalValue = -1 Sensor must Wake up
    the time space between values sent also change (dynamicDelay)
  */
...
void loop() {
  ...
  if (connected) {
    /* code when Sensor is connected*/
    /*LATER: implement reading from the sensor*/
  ...
  delay(dynamicDelay);  // Delay a second between loops.
}
