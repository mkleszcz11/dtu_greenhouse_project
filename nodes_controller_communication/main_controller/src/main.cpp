/*
 * This is the main controller application that connects to the BLE devices
 * (nodes) and recieves/sends messages to them.
 *
 * Code is part of the IoT project for the course "34346 Networking technologies
 * and application development for Internet of Things (IoT)" at the Technical
 * University of Denmark.
 *
 * Code is based on the examples from the ArduinoBLE library, github link ->
...
#define NUMBER_OF_DEVICES \
    3 // Number of devices to connect to (number of nodes) - to have more than 3
      // with Arduino framework drivers alignments are needed.
#define MAX_MESSAGE_SIZE \
    32 // Maximum size of the message from server, if it is larger, it will be
       // truncated. IMPORTANT - messages might be truncated also because of MTU
       // size in BLE settings, it is better to make the message shorter than
       // increase the MTU size.

#define NUMBER_OF_CONTROLLED_PARAMETERS \
    4 // Number of parameters controled - Temperature, Humidity, Solar Radiation
      // and Soil Moisture.

/*State Machine*/
enum States { FIRST_CONNECTION,
    LOW_CONSUMPTION,
    ACTUATOR_MODE };

/* Enum to store the index of the devices, used later in the code to easlily
 * reference to specific defice. */
enum DeviceIndex {
...
uint8_t sensor_info[NUMBER_OF_CONTROLLED_PARAMETERS]; // {soil moisture,
                                                      // temperature, humidity,
                                                      // solar radiation}
uint8_t control_info[3]; // {value_type, desired_value_lower_bound,
                         // desired_value_upper_bound}
...
struct DeviceConnection {
    ...
    BLERemoteCharacteristic* characteristicACK; // Pointer to the characteristic
                                                // to ACK messages of the device.
    ...
    BLEUUID charUUIDACK; // UUID for the characteristic for MainController to
                         // Ackowledge messages
    ...
    // Constructor, initialize the structure with the service and characteristic
    // UUIDs.
    DeviceConnection(const char* serviceUUID, const char* charUUID,
        const char* charUUIDACK)
        : device(nullptr)
        , characteristic(nullptr)
        , doConnect(false)
        , connected(false)
        , serviceUUID(BLEUUID(serviceUUID))
        , charUUID(BLEUUID(charUUID))
        , charUUIDACK(BLEUUID(charUUIDACK))
...
/* Array to store all devices info, it should be mapped accordingly to nodes
 * code. */
DeviceConnection devices[NUMBER_OF_DEVICES] = {
    DeviceConnection(
        "2E01EA51-1E73-4DAC-886D-08C04C040282",
        "beb5483e-36e1-4688-b7f5-ea07361b26a8",
        "beb5483e-36e1-4688-b7f5-ea07361b26a9"), // Soil moisture sensor
    DeviceConnection(
        "9E61059E-B1CF-4953-A632-39CFD6A97255",
        "beb5483e-36e1-4688-b7f5-ea07361b26a8",
        "beb5483e-36e1-4688-b7f5-ea07361b26a9"), // Water pump actuator
    DeviceConnection("A077916E-2CBE-45E3-BB35-4514C322606F",
        "beb5483e-36e1-4688-b7f5-ea07361b26a8",
        "beb5483e-36e1-4688-b7f5-ea07361b26a9") // Humidity sensor
    // DeviceConnection("8FBB11A7-163B-43C8-9D4F-FCA02297A258",
    // "beb5483e-36e1-4688-b7f5-ea07361b26a8")  // Heater
};
...
struct SensorActuatorConfig {
    ...
    // Constructor, initialize the structure with high boundary, low boundary and
    // device index
    SensorActuatorConfig(float lowLimit, float highLimit, DeviceIndex index)
        : highBoundary(highLimit)
        , lowBoundary(lowLimit)
        , actuatorOn(false)
...
enum SensorActuatorIndex {
    ...
    TEMPERATURE_WINDOW, // Temperature and Humidity share the same device and
                        // actuator
...
// Map function used to map the code to parameter index, while receiving message
// about desired values
...
bool send_message_to_device(int deviceIndex, States state)
{
    ...
    if (devices[deviceIndex].characteristic == nullptr)
        return false;
    ...
    devices[deviceIndex].characteristicACK->writeValue(message.c_str(),
        message.length());

    return true;
}

/* Callback function for notify. This callback will be invoked only when the
 * characteristic value is changed. */
static void notifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic,
    uint8_t* p_data, size_t length, bool isNotify)
{
    ...
    for (int i = 0; i < NUMBER_OF_DEVICES; i++) {
        if (devices[i].device != nullptr && devices[i].connected) {
            if (devices[i].device->getAddress().equals(
                    pBLERemoteCharacteristic->getRemoteService()
                        ->getClient()
                        ->getPeerAddress())) {
    ...
    case HUMIDITY_TEMPERATURE_SENSOR:
        ...
        if (controlPairs[HUMIDITY_WINDOW].respectsLimits(
                readValue2)
            && /* Both values respect the boundaries and it's in
                  ACTUATOR MODE*/
            controlPairs[TEMPERATURE_WINDOW].respectsLimits(readValue1) && devices[deviceIndexNotification].currentState == ACTUATOR_MODE) {
            devices[deviceIndexNotification].changeState(LOW_CONSUMPTION);
            Serial.println(
                String("[Main Controller] Transition Device ") + devices[deviceIndexNotification].device->getName().c_str());
        } else if ((!controlPairs[HUMIDITY_TEMPERATURE_SENSOR].respectsLimits(
                        readValue2)
                       || !controlPairs[TEMPERATURE_WINDOW].respectsLimits(readValue1))
            && devices[deviceIndexNotification].currentState == LOW_CONSUMPTION) /*One of the values does not respect the
                                                                                    boundaries and it's in LOW_CONSUMPTION*/
        {
    ...
    default: // controlPair index is the same as device
        ...
        if (controlPairs[deviceIndexNotification].respectsLimits(
                readValue1)
            && /* value respect the boundaries and it's in ACTUATOR
                  MODE*/
            ...
            Serial.println(
                String("[Main Controller] Transition Device ") + devices[deviceIndexNotification].device->getName().c_str());
        } else if (!controlPairs[deviceIndexNotification].respectsLimits(
                       readValue1)
            && devices[deviceIndexNotification].currentState == LOW_CONSUMPTION) /* value does not respect the boundaries and
                                                                                    it's in LOW_CONSUMPTION*/
        {
            devices[deviceIndexNotification].currentState = ACTUATOR_MODE;
            devices[deviceIndexNotification].transition = true;
            Serial.println(
    ...
    void onConnect(BLEClient* pclient) override
    {
        ...
        for (int i = 0; i < NUMBER_OF_DEVICES; i++) {
            if (devices[i].device != nullptr && devices[i].device->getAddress() == pclient->getPeerAddress()) { // Check if the device is in the list.
                ...
                // Serial.println(String("[Main Controller] - Device connected: ") +
                // devices[i].device->getName().c_str());
                break;
            }
        }
    }
    ...
    void onDisconnect(BLEClient* pclient) override
    {
        ...
        for (int i = 0; i < NUMBER_OF_DEVICES; i++) {
            if (devices[i].device != nullptr && devices[i].device->getAddress() == pclient->getPeerAddress()) { // Check if the device is in the list.
                ...
                print_all_devices_flag = true; // Set the flag to print all devices when
                                               // the will be connected again.
                devices[i].connected = false;
                devices[i].doConnect = false; // Update the corresponding device status.
                // Serial.println(String("[Main Controller] - Device disconnected: ") +
                // devices[i].device->getName().c_str());
...
bool connectToServer(BLEAdvertisedDevice* myDevice, int deviceIndex)
{
    ...
    if (pRemoteCharacteristic
            ->canRead()) { // If the characteristic can be read, read it.
        std::string value = pRemoteCharacteristic->readValue();
    }

    if (pRemoteCharacteristic->canNotify()) // If the characteristic can notify,
                                            // go to notify callback.
 ...
 * If the device is found, it will stop the scan and try to connect to the
 * device.
 */
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
    ...
    {
        for (int i = 0; i < NUMBER_OF_DEVICES; i++) {
            if (advertisedDevice.haveServiceUUID() && advertisedDevice.isAdvertisingService(devices[i].serviceUUID)) {
                if (devices[i].device == nullptr || !devices[i].connected) {
                    // Serial.println(String("[Main Controller] Found a device: ") +
...
void loop()
{
    ...
    // /*********************************************************************************/
    // /*** LORA - RECEIVING DESIRED PARAMETERS BOUDARIES VALUES FROM THE REMOTE
    // NODE ***/
    // /*********************************************************************************/
    ...
    if (!lora_transmit_mode_flag && !lora_receive_mode_flag) {
        ...
        lora_receive(control_info, 3, &lora_receive_mode_flag,
            &lora_receive_message_flag);
    }

    // Check for new messages every 5 seconds
    if (lora_receive_mode_flag && current_millis - last_receive_time >= 5000) {
        check_for_incoming_message(
            control_info, 3, &lora_receive_mode_flag,
            &lora_receive_message_flag); // Check if there is a message that waits
                                         // to be processed.
    ...
    if (current_millis - last_sent_time >= 10000) { // current_millis - last_sent_time >= 10000 &&
        ...
        check_for_incoming_message(
            control_info, 3, &lora_receive_mode_flag,
            &lora_receive_message_flag); // Firstly, check if there is a message
                                         // that waits to be processed.
        lora_transmit_mode_flag = true;
        lora_transmit(sensor_info, NUMBER_OF_CONTROLLED_PARAMETERS,
            &lora_transmit_mode_flag, &lora_receive_mode_flag,
            &lora_receive_message_flag);
    ...
    if (connectedDevices == NUMBER_OF_DEVICES && print_all_devices_flag && startSystem) {
        ...
        startSystem = false; // it will never be true again (if we decide to sleep
                             // MainController this must change)
    }
    ...
    if (connectedDevices < NUMBER_OF_DEVICES) {
        BLEDevice::getScan()->start(
            5, false); // Rescan for 5 seconds (it has blocking behavior)
        // Serial.println("[Main Controller] Scanning for devices, connected
        // devices: "
    ...
    if (connectedDevices == NUMBER_OF_DEVICES && current_millis - previous_millis > interval) {
        Serial.println(
            "[Main Controller] Connected devices: " + String(connectedDevices) + " / " + String(NUMBER_OF_DEVICES));
    ...
    if (!startSystem) {
        for (int i = 0; i < NUMBER_OF_DEVICES; i++) {
            ...
            // Serial.println(String("[Main Controller] ACK FLAG : ") +
            // devices[i].messageACK );
            if (devices[i].messageACK) {
                if (send_message_to_device(i, devices[i].currentState)) {
                    ...
                    Serial.println(
                        String("[Main Controller] Error: Message not sent to Device ") + devices[i].device->getName().c_str());
                }
            }
        }
    }
