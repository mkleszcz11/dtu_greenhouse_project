...
#define NUMBER_OF_CONTROLLED_PARAMETERS 4  // Number of parameters controled - Temperature, Humidity, Water Level and Soil Moisture.
...
uint8_t sensor_info[NUMBER_OF_CONTROLLED_PARAMETERS];  // {soil moisture, temperature, humidity, water level}
uint8_t control_info[2];                               // {value_type, value}
...
enum SensorActuatorIndex {
  ...
  WATER_LEVEL_LIGHT     // SensorActuator index matches with device index
};
...
static void notifyCallback(
  ...
  switch (deviceIndexNotification) {
    case HUMIDITY_TEMPERATURE_SENSOR:
      ...
      } else if ((!controlPairs[HUMIDITY_TEMPERATURE_SENSOR].respectsLimits(readValue2) || !controlPairs[TEMPERATURE_WINDOW].respectsLimits(readValue1)) && devices[deviceIndexNotification].currentState == LOW_CONSUMPTION) /*One of the values does not respect the boundaries and it's in LOW_CONSUMPTION*/
      {
    ...
    default:  //controlPair index is the same as device
      ...
      if (controlPairs[deviceIndexNotification].respectsLimits(readValue1) && /* value respect the boundaries and it's in ACTUATOR MODE*/
        ...
        Serial.println(String("[Main Controller] Transition Device ") + devices[deviceIndexNotification].device->getName().c_str());
      } else if (!controlPairs[deviceIndexNotification].respectsLimits(readValue1) && devices[deviceIndexNotification].currentState == LOW_CONSUMPTION) /* value does not respect the boundaries and it's in LOW_CONSUMPTION*/
      {
        devices[deviceIndexNotification].currentState = ACTUATOR_MODE;
        devices[deviceIndexNotification].transition = true;
        Serial.println(String("[Main Controller] Transition Device ") + devices[deviceIndexNotification].device->getName().c_str());
  ...
  switch (deviceIndexNotification) {
    ...
    case WATER_LEVEL_SENSOR:
      sensor_info[VAL_WATER_LEVEL] = readValue1;
      break;
    default:
      break;
  }
...
void loop() {
  ...
  if (!lora_transmit_mode_flag && !lora_receive_mode_flag) {
    ...
    lora_receive(control_info, 2, &lora_receive_mode_flag, &lora_receive_message_flag);
  }

  // Check for new messages every 5 seconds
  if (lora_receive_mode_flag && current_millis - last_receive_time >= 5000) {
    check_for_incoming_message(control_info, 2, &lora_receive_mode_flag, &lora_receive_message_flag);  // Check if there is a message that waits to be processed.
  ...
  if (current_millis - last_sent_time >= 10000) {                                                      //current_millis - last_sent_time >= 10000 && !lora_receive_mode_flag) {
    ...
    check_for_incoming_message(control_info, 2, &lora_receive_mode_flag, &lora_receive_message_flag);  // Firstly, check if there is a message that waits to be processed.
